...
for filename in glob.iglob(str(directory_name) + '/*/*' + str(extension_type), recursive=True):
    ...
    try:
        ...
        coco_list_energy_all = coco_list_energy_all.astype(float)

        coco_list_energy_all_meaned = coco_list_energy_all.groupby(['mass']).mean()
        
        coco_list_energy_all_meaned = coco_list_energy_all_meaned.reset_index()
        
        
        coco_list_energy_all = coco_list_energy_all_meaned.sort_values('mass', ascending = True)
